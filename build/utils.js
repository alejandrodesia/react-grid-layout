...
exports.cloneLayoutItemByIndex = cloneLayoutItemByIndex;
...
function cloneLayoutItemByIndex(layout /*: Layout*/, index /*: number*/) /*: LayoutItem*/{
  const layoutItem = cloneLayoutItem(layout[index]);
  layout[index] = layoutItem;
  return layoutItem;
}
...
function compact(layout /*: Layout*/, compactType /*: CompactType*/, cols /*: number*/, allowOverlap /*: ?boolean*/) /*: Layout*/{
  ...
  for (let i = 0, len = sorted.length; i < len; i++) {
    ...
    let index = layout.indexOf(sorted[i]);
    if (index === -1) index = layout.findIndex(item => item.i === sorted[i].i);
    out[index] = l;
...
function resolveCompactionCollision(layout /*: Layout*/, item /*: LayoutItem*/, moveToCoord /*: number*/, axis /*: "x" | "y"*/) {
  ...
  for (let i = itemIndex + 1; i < layout.length; i++) {
    const otherItem = cloneLayoutItemByIndex(layout, i);
    // Ignore static items
...
function getLayoutItem(layout /*: Layout*/, id /*: string*/) /*: ?LayoutItem*/{
  for (let i = 0, len = layout.length; i < len; i++) {
    if (layout[i].i === id) {
      const l = cloneLayoutItem(layout[i]);
      layout[i] = l;
      return layout[i];
    }
  }
